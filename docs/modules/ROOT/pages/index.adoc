...
bean implementing the `ContextPropagator` interface.
...
To enable interceptors simply produce a CDI bean implementing the `WorkflowClientInterceptor` interface or `WorkerInterceptor` interface.
