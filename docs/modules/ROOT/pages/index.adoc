...
public class MyService {
    ...
    public void addRecipient(WorkflowData data, String email) {
        ...
        WorkflowExecution execution = WorkflowStub.fromTyped(workflow) // <2>
                .signalWithStart("addRecipient", new Object[] { email }, new Object[] { data }); // <3>
    }
...
<2> The unwrapped stub carries the task queue, reuse policy, conflict policy, retry options and timeouts configured under `quarkus.temporal.workflow.*`
<3> The first array holds the signal arguments, the second one the arguments of the `@WorkflowMethod` used if the workflow has to be started
