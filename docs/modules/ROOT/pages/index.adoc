...
Temporal Dev Services starts a local Temporal development server with Docker or Podman when no `quarkus.temporal.connection.target` is configured. It is enabled by default in dev and test modes and automatically supplies the dynamically mapped gRPC address to the Temporal client.
...
The first application starts `temporalio/temporal:latest`; later applications in dev mode reuse it by default. The Temporal card in the Quarkus Dev UI includes a link to the Temporal Web UI.
