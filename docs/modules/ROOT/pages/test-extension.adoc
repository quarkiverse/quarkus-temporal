...
A `@QuarkusTest` can then inject the `org.jboss.logging.Logger`, `WorkflowClient`, and `WorkerFactory` beans, both of which are backed by the test environment:
...
public class QuarkusBasedOrderEntityWorkflowTest {
    ...
    @ConfigProperty(name = "quarkus.temporal.worker.\"cas-rate-limited\".task-queue")
    String rateLimitedTaskQueue;
    ...
    public void testHappyPathOrderProcessing() {
        ...
        // <3> Register the mocks with the worker that owns each activity.
        ...
        // <4> Create a typed workflow stub backed by the in-memory test server.
        ...
        try {
            ...
            assertTrue(result.contains("order status FULLFILLMENT"), "Order status should be FULLFILLMENT");
        } finally {
            // <9> Clean up.
